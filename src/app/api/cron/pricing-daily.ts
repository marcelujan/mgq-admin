...
/**
 * Implementá el "motor por proveedor" acá.
 * Input: motorId + url
 * Output: precio ARS + url efectiva (por si normalizás/canónica)
 */
async function scrapeWithMotor(motorId: number, url: string): Promise<{ priceArs: number; sourceUrl: string }> {
  // TODO: conectar con tu capa existente de motores.
  // Por ahora: tirar error explícito si aún no está implementado.
