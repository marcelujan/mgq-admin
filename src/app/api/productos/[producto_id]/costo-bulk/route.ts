...
async function computeBulkCost(sql: any, producto_id: number, stack: number[], depth: number): Promise<BulkCost> {
  ...
  // Resolver preferente: si existe snapshot diario para el BULK formulado, usarlo.
  // Motivo: un BULK puede ser usado como componente (BULK_PRODUCTO) y su costo fuente de verdad es el snapshot.
  // Esto evita fallos cuando el producto referenciado no tiene aún fórmula v2 completa, pero sí snapshot calculado por cron.
  try {
    const fIdRes: any = await sql.query(
      `SELECT item_formulado_id
       FROM app.item_formulado
       WHERE producto_id=$1 AND tipo='BULK' AND activo=true
       ORDER BY item_formulado_id ASC
       LIMIT 1`,
      [producto_id]
    );
    const fIdRows = normalizeQueryResult(fIdRes);
    const item_formulado_id = fIdRows?.[0]?.item_formulado_id ? Number(fIdRows[0].item_formulado_id) : null;

    if (item_formulado_id) {
      const sRes: any = await sql.query(
        `SELECT precio_unitario_ars
         FROM app.item_formulado_snapshot
         WHERE item_formulado_id=$1 AND as_of_date=current_date
         ORDER BY created_at DESC
         LIMIT 1`,
        [item_formulado_id]
      );
      const sRows = normalizeQueryResult(sRes);
      const precio = sRows?.[0]?.precio_unitario_ars !== undefined ? Number(sRows[0].precio_unitario_ars) : null;

      if (precio !== null && Number.isFinite(precio) && precio > 0) {
        // lote_ref_g: intentar leerlo si existe; sino default 1000.
        let lote_ref_g = 1000;
        try {
          const fRes2: any = await sql.query(
            `SELECT lote_ref_g FROM app.producto_formula_v2 WHERE producto_id=$1`,
            [producto_id]
          );
          const fRows2 = normalizeQueryResult(fRes2);
          const lr = fRows2?.[0]?.lote_ref_g ? Number(fRows2[0].lote_ref_g) : null;
          if (lr !== null && Number.isFinite(lr) && lr > 0) lote_ref_g = lr;
        } catch {
          // ignore
        }
        ...
        return {
          ars_por_kg: precio,
          ars_por_g: precio / 1000,
          lote_ref_g,
          prod_ars_por_kg: 0,
          material_ars_por_kg: precio,
        };
      }
    }
  } catch {
    // Si falla la lectura, continuar con cálculo por fórmula.
  }
