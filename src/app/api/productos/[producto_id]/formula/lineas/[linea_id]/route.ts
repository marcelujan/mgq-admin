...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ producto_id: string; linea_id: string }> }) {
  try {
    ...
    if (body?.pct_peso !== undefined) {
      ...
      if (!Number.isFinite(p) || p < 0 || p > 100) return NextResponse.json({ ok: false, error: "pct_peso inválido" }, { status: 422 });
      pushSet("pct_peso = ?", p);
    }
    ...
    if (body?.insumo_id !== undefined) {
      ...
      if (!Number.isFinite(insumo_id) || insumo_id <= 0) return NextResponse.json({ ok: false, error: "insumo_id inválido" }, { status: 422 });
      // Validar no UN
      const t: any = await sql.query(`SELECT tipo_uom FROM app.insumo WHERE insumo_id=$1 LIMIT 1`, [insumo_id]);
      const tipo_uom = t?.rows?.[0]?.tipo_uom ?? (Array.isArray(t) ? t?.[0]?.tipo_uom : null);
      if (!tipo_uom) return NextResponse.json({ ok: false, error: "insumo no existe" }, { status: 422 });
      if (String(tipo_uom).toUpperCase() === "UN") return NextResponse.json({ ok: false, error: "Insumo UN no puede estar en % p/p" }, { status: 422 });
...
export async function DELETE(_: NextRequest, ctx: { params: Promise<{ producto_id: string; linea_id: string }> }) {
  try {
    ...
    const { linea_id: linea_idStr, producto_id: producto_idStr } = await ctx.params;
const sql = db();
    const producto_id = Number(producto_idStr);
    const linea_id = Number(linea_idStr);
    if (!Number.isFinite(producto_id) || !Number.isFinite(linea_id)) {
