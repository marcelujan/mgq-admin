...
export async function GET(req: Request) {
  try {
    ...
    // Vista "últimos SUCCEEDED" por item_id (útil para tabla operativa)
    if (latestSucceeded) {
      const rows = await sql`
        SELECT DISTINCT ON (j.item_id)
          ...
          COALESCE(jsonb_array_length(jr.result->'warnings'), 0)::int AS warnings_count,
          COALESCE(jsonb_array_length(jr.result->'errors'), 0)::int AS errors_count,
          NULLIF((jr.result->>'valid_count')::int, NULL) AS valid_count,
