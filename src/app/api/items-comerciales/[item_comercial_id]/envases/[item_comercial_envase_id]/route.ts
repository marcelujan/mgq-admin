...
export async function PATCH(req: NextRequest, ctx: { params: { item_comercial_id: string; item_comercial_envase_id: string } }) {
  try {
    ...
    values.push(item_comercial_id, item_comercial_envase_id);

    const sql = db();
    const r: any = await sql.query(
      `
      UPDATE app.item_comercial_envase
      SET ${sets.join(", ")}, updated_at = now()
      WHERE item_comercial_id = $${p++} AND item_comercial_envase_id = $${p}
    ...
    return NextResponse.json({ ok: true, item_comercial_envase_id: row.item_comercial_envase_id });
  } catch (e: any) {
    return NextResponse.json({ ok: false, error: e?.message ?? "error" }, { status: 500 });
  }
}

export async function DELETE(_: NextRequest, ctx: { params: { item_comercial_id: string; item_comercial_envase_id: string } }) {
  try {
    const item_comercial_id = Number(ctx.params.item_comercial_id);
    const item_comercial_envase_id = Number(ctx.params.item_comercial_envase_id);
    if (!Number.isFinite(item_comercial_id) || item_comercial_id <= 0 || !Number.isFinite(item_comercial_envase_id) || item_comercial_envase_id <= 0) {
    ...
    const sql = db();
    const r: any = await sql.query(
      `DELETE FROM app.item_comercial_envase WHERE item_comercial_id = $1 AND item_comercial_envase_id = $2 RETURNING item_comercial_envase_id`,
      [item_comercial_id, item_comercial_envase_id]
    );
    const row = rowsOf(r)[0] ?? null;
    if (!row) return NextResponse.json({ ok: false, error: "No encontrado" }, { status: 404 });
    return NextResponse.json({ ok: true });
