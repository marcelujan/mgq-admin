...
export async function POST(req: NextRequest) {
  try {
    ...
    if (cantidad === null || cantidad <= 0) return NextResponse.json({ ok: false, error: "cantidad inválida" }, { status: 400 });
    ...
    const op = await createStockOperacion({ tipo: "INGRESO", fecha, nota, referencia_externa });
    if (!op) throw new Error("No se pudo crear stock_operacion");
    await insertStockMovimiento({ stock_operacion_id: op.stock_operacion_id, item_tipo, item_ref_id, delta_cantidad: cantidad, nota });
