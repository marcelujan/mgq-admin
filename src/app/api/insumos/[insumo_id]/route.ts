...
export async function GET(_: NextRequest, ctx: { params: Promise<{ insumo_id: string }> }) {
  try {
    ...
    const insumo_id = Number(paramsObj.insumo_id);
    ...
    const insumo = normalizeQueryResult(a)?.[0];
    ...
    const fuentes = normalizeQueryResult(b);

    return NextResponse.json({ ok: true, insumo, fuentes });
  } catch (e: any) {
