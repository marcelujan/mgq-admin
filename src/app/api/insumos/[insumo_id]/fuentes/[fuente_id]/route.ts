...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ insumo_id: string; fuente_id: string }> }) {
  try {
    ...
    if (body?.item_id !== undefined) {
      ...
      if (!Number.isFinite(item_id) || item_id <= 0) return NextResponse.json({ ok: false, error: "item_id inválido" }, { status: 422 });
      pushSet("item_id = ?", item_id);
    }
...
export async function DELETE(_req: NextRequest, ctx: { params: Promise<{ insumo_id: string; fuente_id: string }> }) {
  try {
    const sql = db();
    const { insumo_id: insumoIdStr, fuente_id: fuenteIdStr } = await ctx.params;
    const insumo_id = Number(insumoIdStr);
