...
export async function PATCH(req: NextRequest, ctx: { params: { insumo_id: string; fuente_id: string } }) {
  try {
    const sql = db();
    const insumo_id = Number(ctx.params.insumo_id);
    const fuente_id = Number(ctx.params.fuente_id);
...
export async function DELETE(_: NextRequest, ctx: { params: { insumo_id: string; fuente_id: string } }) {
  try {
    const sql = db();
    const insumo_id = Number(ctx.params.insumo_id);
    const fuente_id = Number(ctx.params.fuente_id);
