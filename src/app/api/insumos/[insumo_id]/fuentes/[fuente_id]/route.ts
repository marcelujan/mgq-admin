...
export async function PATCH(
  req: NextRequest,
  ctx: { params: Promise<{ insumo_id: string; fuente_id: string }> }
) {
  try {
    const sql = db();
    const { insumo_id: insumoIdStr, fuente_id: fuenteIdStr } = await ctx.params;

    const insumo_id = Number(insumoIdStr);
    const fuente_id = Number(fuenteIdStr);
    ...
    if (body?.vigente_desde !== undefined) {
      pushSet("vigente_desde = ?", typeof body.vigente_desde === "string" ? body.vigente_desde : null);
    }

    if (body?.presentacion_preferida !== undefined) {
      const pref = Number(body.presentacion_preferida);
      if (!Number.isFinite(pref) || pref <= 0) {
        return NextResponse.json({ ok: false, error: "presentacion_preferida inválida" }, { status: 422 });
      }
    ...
    const q = `UPDATE app.insumo_fuente
              SET ${sets.join(", ")}
              WHERE insumo_id=$${params.length - 1} AND fuente_id=$${params.length}`;
