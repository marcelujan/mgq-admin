...
// Producto base: define si el producto es "BASE" (sin fórmula) y de qué está hecho.
// Políticas:
// - tipo_base: 'INSUMO' | 'ITEM'
// - Si INSUMO => insumo_id requerido
// - Si ITEM   => item_id + presentacion_preferida requeridos (preferida obligatoria)
//
// Nota: este endpoint asume que existe la tabla app.producto_base (producto_id PK/FK).
...
export async function GET(
  ...
  try {
    ...
    const r: any = await sql.query(
      `SELECT producto_id, tipo_base, insumo_id, item_id, presentacion_preferida, created_at, updated_at
    ...
    if (tipo_base === "INSUMO") {
      ...
      if (!Number.isFinite(presentacion_preferida) || (presentacion_preferida as number) <= 0) {
        return NextResponse.json(
          { ok: false, error: "presentacion_preferida requerida (>0) para tipo_base=ITEM" },
          { status: 422 }
        );
      }
    }

    // Upsert por producto_id
    const q = `
