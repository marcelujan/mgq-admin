...
type ComercialEstado = "BORRADOR" | "OFERTABLE" | "CON_FALTANTES" | "BLOQUEADO";
...
type EstadoFiltro = "" | ComercialEstado;
...
function colorOf(estado: ComercialEstado) {
  if (estado === "OFERTABLE") return "#22c55e";
  if (estado === "CON_FALTANTES") return "#f2c94c";
...
export default function StockFaltantesClient() {
  ...
  const summary = useMemo(() => ({
    borrador: comerciales.filter((x) => x.estado === "BORRADOR").length,
    ofertable: comerciales.filter((x) => x.estado === "OFERTABLE").length,
    conFaltantes: comerciales.filter((x) => x.estado === "CON_FALTANTES").length,
    bloqueado: comerciales.filter((x) => x.estado === "BLOQUEADO").length,
  }), [comerciales]);
    ...
    <div style={{ display: "grid", gap: 14 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
        ...
        <select value={estado} onChange={(e) => setEstado(e.target.value as EstadoFiltro)} style={{ border: "1px solid rgba(255,255,255,0.14)", borderRadius: 10, padding: "8px 10px", background: "rgba(255,255,255,0.03)", color: "inherit" }}>
          ...
          <option value="BORRADOR">Gris · incompleto</option>
          <option value="OFERTABLE">Verde · ofertable</option>
          <option value="CON_FALTANTES">Amarillo · con faltantes</option>
          <option value="BLOQUEADO">Rojo · sin bulk</option>
        </select>
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", fontSize: 12, opacity: 0.8 }}>
        <div>Gris: <b>{summary.borrador}</b></div>
        <div>Verde: <b>{summary.ofertable}</b></div>
        <div>Amarillo: <b>{summary.conFaltantes}</b></div>
        <div>Rojo: <b>{summary.bloqueado}</b></div>
      </div>
        ...
        <div style={{ border: "1px solid rgba(255,255,255,0.10)", borderRadius: 14, overflow: "hidden" }}>
          <div style={{ maxHeight: 34 + 10 * 29, overflowY: "auto", overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12.5, tableLayout: "fixed" }}>
              <thead>
                <tr style={{ background: "rgba(255,255,255,0.03)" }}>
                  <th style={{ textAlign: "left", padding: "5px 8px", width: 55 }}>Estado</th>
                  <th style={{ textAlign: "left", padding: "5px 8px", width: 80 }}>Item #</th>
                  <th style={{ textAlign: "left", padding: "5px 8px" }}>Nombre</th>
                  <th style={{ textAlign: "left", padding: "5px 8px", width: 90 }}>UOM</th>
                  ...
                  <th style={{ textAlign: "left", padding: "5px 8px", width: 240 }}>Detalle</th>
                  <th style={{ textAlign: "left", padding: "5px 8px", width: 90 }}></th>
                </tr>
              </thead>
              <tbody>
                {comerciales.map((r) => (
                  <tr key={r.item_comercial_id} style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                    <td style={{ padding: "5px 8px" }}><span title={r.estado} style={{ display: "inline-flex", width: 10, height: 10, borderRadius: 999, background: colorOf(r.estado) }} /></td>
                    <td style={{ padding: "5px 8px", whiteSpace: "nowrap" }}>{r.item_comercial_id}</td>
                    <td style={{ padding: "5px 8px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }} title={r.nombre ?? ""}>{r.nombre ?? ""}</td>
                    <td style={{ padding: "5px 8px", whiteSpace: "nowrap" }}>{r.unidad ?? ""}</td>
                    <td style={{ padding: "5px 8px", textAlign: "right", whiteSpace: "nowrap" }}>{fmt(r.bulk_requerido)}</td>
                    <td style={{ padding: "5px 8px", textAlign: "right", whiteSpace: "nowrap" }}>{fmt(r.bulk_saldo)}</td>
                    <td style={{ padding: "5px 8px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }} title={[r.estado_detalle, ...r.warnings_detalle].filter(Boolean).join("\n")}>{r.estado_detalle}</td>
                    <td style={{ padding: "5px 8px", whiteSpace: "nowrap" }}><Link href={`/items-comerciales/${r.item_comercial_id}`} style={actionLink}>Abrir</Link></td>
                  </tr>
                ))}
