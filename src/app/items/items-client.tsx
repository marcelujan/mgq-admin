...
type ItemRow = {
  ...
  provider_item_nombre?: string | null;
  provider_item_codigo?: string | null;
...
function providerFallbackName(providerName: string, providerItemCode: string, url: string): string | null {
  const code = providerItemCode.trim();
  if (providerName && code) return `${providerName} · SKU ${code}`;

  const fromUrl = nameFromUrl(url);
  if (!fromUrl) return providerName || null;

  const normalized = fromUrl.trim();
  if (!normalized) return providerName || null;
  if (/^products?_?id$/i.test(normalized)) return providerName || null;
  if (/^\d+$/.test(normalized)) return providerName && code ? `${providerName} · SKU ${code}` : providerName || null;
  if (/^oscsid$/i.test(normalized)) return providerName || null;
  return normalized;
}
        ...
        <table style={{ minWidth: 1220, width: "100%", fontSize: 13, borderCollapse: "collapse", tableLayout: "fixed" }}>
          ...
          <tbody>
            {items.map((it) => {
              ...
              const fuente = String(it.proveedor_nombre ?? "—").trim() || "—";
              const providerItemName = String(it.provider_item_nombre ?? "").trim();
              const providerItemCode = String(it.provider_item_codigo ?? "").trim();

              let nombre =
                (kind === "FORMULADO" ? String(it.producto_nombre ?? "").trim() : "") ||
                (kind === "MANUAL" ? String(it.manual_nombre ?? "").trim() : "") ||
                (isProv ? providerItemName : "") ||
                (isProv ? providerFallbackName(fuente === "—" ? "" : fuente, providerItemCode, url) ?? "" : "") ||
                (url ? url : "");
