...
export default function Page(){
  ...
  // recargar al cambiar “Solo habilitados” (UX)
  useEffect(()=>{ search(); /* eslint-disable-next-line react-hooks/exhaustive-deps */ }, [onlyEnabled]);
  ...
  async function toggleInterest(pid:number,val:boolean){
    await fetch('/api/interest',{method:'PATCH',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({productId:pid, interest:val})});
    ...
    <main className="p-4 max-w-7xl mx-auto space-y-3">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        <label className="px-2"><input type="checkbox" checked={onlyEnabled}
               onChange={e=>setOnlyEnabled(e.target.checked)}/> Solo habilitados</label>
        <label className="px-2"><input type="checkbox" checked={onlyInterest}
               onChange={e=>setOnlyInterest(e.target.checked)}/> Solo interés</label>
        <label className="px-2"><input type="checkbox" checked={hasCost}
               onChange={e=>setHasCost(e.target.checked)}/> Con costo</label>
        <label className="px-2"><input type="checkbox" checked={hasPrice}
               onChange={e=>setHasPrice(e.target.checked)}/> Con sugerido</label>
              ...
              <tr key={r.product_presentation_id} className="border-t">
                ...
                <td className="p-2 text-center">
                  <input type="checkbox"
                         ...
                         onChange={e=>toggleSelect(r.product_id, e.target.checked)} />
                </td>
                <td className="p-2">
                  <button className="mr-2" title="Interés"
                          onClick={()=>toggleInterest(r.product_id, !(r.interest??false))}>
                    {(r.interest??false)?'★':'☆'}
                  </button>
                  {r.nombre}
                </td>
